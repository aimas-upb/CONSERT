...
parser.add_argument('--f', metavar='INPUT_FOLDER', type=str, nargs=1, required=True,
                    help='folder containin  g the CONSERT knowledgebase exported in JSON files (usually one per each'
                         'entrypoint')
parser.add_argument('--o', metavar='OUTPUT_HTML', type=str, nargs=1, default=['casas-kb-visualizer.html'],
...
df = df[['sensorId', 'annotations.startTime', 'annotations.endTime', 'Event Type']]
...
df['annotations.endTime'] = pd.to_datetime(df['annotations.endTime'] + 1000, unit='ms')

# Rename columns to match plotly conventions
df.rename(columns={'sensorId': 'Task', 'annotations.startTime': 'Start', 'annotations.endTime': 'Finish'}, inplace=True)
